...
public class CheckOutActivity extends RFIDReaderBaseActivity {
    ...
    private ArrayAdapter<Laboratory> arrayAdapter;
    private List<Laboratory> departments = new ArrayList<>();
    private LaboratoryRepository laboratoryRepo;
    private Calendar calendar = Calendar.getInstance();
    private Laboratory selected;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initViews();
        setTitle(getResources().getString(R.string.label_asset_check_out));
                ...
                if (asset != null) {
                    ...
                    if (selected != null) {
                        ...
                        if (asset.getDepartment() == Constraints.CHECK_IN_DEPARTMENT) {
                            asset.setDepartment(selected.getLabId());
                            checkOut(payload);
                        } else {
                            AppAlertDialog.showErrorMessage(CheckOutActivity.this, "Asset already in Checked-OUT status", null);
                        }


                    } else {
                        showToast("Select department to checkout");
                    }
        ...
        laboratoryRepo = new LaboratoryRepository(app);
        laboratoryRepo.getAll().observe(this, new Observer<List<Laboratory>>() {
            @Override
            public void onChanged(List<Laboratory> laboratories) {
                departments = laboratories;

                arrayAdapter = new ArrayAdapter<Laboratory>(CheckOutActivity.this, android.R.layout.simple_spinner_item, departments);
                arrayAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                spinnerCheckOut.setAdapter(arrayAdapter);
            }
        });


        spinnerCheckOut.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                selected = departments.get(position);
            }
        ...
        ApiClient.getApiService().assetDetails(session.token(), inventory.getFormattedEPC()).enqueue(new Callback<AssetResponse>() {
            ...
            public void onResponse(Call<AssetResponse> call, Response<AssetResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    ApiUtils.processApiError(CheckOutActivity.this, response);
                }
            ...
            public void onFailure(Call<AssetResponse> call, Throwable t) {
                progressBar.setVisibility(View.GONE);
                ApiUtils.processApiFailure(CheckOutActivity.this, t);
            }
        });
    }

    private void checkOut(SyncPayload payload) {
        ...
        ApiClient.getApiService().updateAssetsNew(session.token(), payload).enqueue(new Callback<SyncPayload>() {
            ...
            public void onResponse(Call<SyncPayload> call, Response<SyncPayload> response) {
                if (response.isSuccessful()) {
                    logger.i(TAG, "Checked-Out Success " + response.code());

                    AppAlertDialog.showSuccessMessage(CheckOutActivity.this, "Asset Checked Out", new AppAlertDialog.ClickListener() {
                        @Override
                        public void onClick() {
                            finish();
                        }
                    ...
                    logger.e(TAG, "Checked-Out Failed");
                    asset.setDepartment(assetDepartmentId);
